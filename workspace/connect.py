...
class CONNECT(object):
    ...
    def setup_pozyx(self):
        ...
        if self.remote_id is None:
            for device_id in [self.remote_id, self.dest_id]:
                self.pypozyx.printDeviceInfo(device_id)
        else:
            for device_id in [None, self.remote_id, self.dest_id]:
        ...
        led_config = 0x0
        self.pypozyx.setLedConfig(led_config, self.remote_id)
        # do the same for the destination.
        self.pypozyx.setLedConfig(led_config, self.dest_id)
        # set the ranging protocol
        self.pypozyx.setRangingProtocol(self.protocol, self.remote_id)

    def pozyx_loop(self):
        device_range = DeviceRange()
        status = self.pypozyx.doRanging(
            self.dest_id, device_range, self.remote_id)
        ...
        if status == POZYX_SUCCESS:
            ...
            status = self.pypozyx.getErrorCode(error_code)
            if status == POZYX_SUCCESS:
                print("ERROR Ranging, local %s" %
                      self.pypozyx.getErrorMessage(error_code))
            else:
    ...
    def ledControl(self, distance):
        ...
        ids = [self.remote_id, self.dest_id]
        # set the leds of both local/remote and destination pozyx device
        for id in ids:
            status &= self.pypozyx.setLed(4, (distance < self.range_step_mm), id)
            status &= self.pypozyx.setLed(3, (distance < 2 * self.range_step_mm), id)
            status &= self.pypozyx.setLed(2, (distance < 3 * self.range_step_mm), id)
            status &= self.pypozyx.setLed(1, (distance < 4 * self.range_step_mm), id)
        return status
